...
double calculate_rescale(struct max_data *data) {
	...
	#pragma omp parallel default(shared) private(r, t)
	{
		...
		for (r = 0 ; r < quat->num_rot_p ; ++r) {
			...
			total += quat->quat[(r*param->num_proc + param->rank)*5 + 4] * data->u[r] ;
		}
	...
	for (r = 0 ; r < quat->num_rot_p ; ++r) {
		data->u[r] = log(quat->quat[(r*param->num_proc + param->rank)*5 + 4]) - data->u[r] ;
	}
			...
			if (curr->type == 0) {
				...
				for (t = 0 ; t < curr->ones[d] ; ++t) {
					...
					if (det[detn].mask[pixel] < 1)
						prob[curr->num_data_prev+d] += priv->view[detn][pixel] ;
				}
				...
				for (t = 0 ; t < curr->multi[d] ; ++t) {
					...
					if (det[detn].mask[pixel] < 1)
						prob[curr->num_data_prev+d] += curr->count_multi[curr->multi_accum[d] + t] * priv->view[detn][pixel] ;
				}
			}
			else if (curr->type == 1) {
				for (t = 0 ; t < det[detn].num_pix ; ++t)
				if (det[detn].mask[t] < 1)
					prob[curr->num_data_prev+d] += curr->int_frames[d*curr->num_pix + t] * priv->view[detn][t] ;
			}
			else if (curr->type == 2) { // Gaussian EMC for double precision data without scaling
				for (t = 0 ; t < det[detn].num_pix ; ++t)
				if (det[detn].mask[t] < 1)
					prob[curr->num_data_prev+d] -= pow(curr->frames[d*curr->num_pix + t] - priv->view[detn][t]*iter->rescale, 2.) ;
			}
			...
			if (curr->type == 0) {
				...
				for (t = 0 ; t < curr->ones[d] ; ++t) {
					...
					if (det[detn].mask[pixel] < 2)
						priv->view[detn][pixel] += prob[curr->num_data_prev+d] ;
				}
				...
				for (t = 0 ; t < curr->multi[d] ; ++t) {
					pixel = curr->place_multi[curr->multi_accum[d] + t] ;
					if (det[detn].mask[pixel] < 2)
						priv->view[detn][pixel] += curr->count_multi[curr->multi_accum[d] + t] * prob[curr->num_data_prev+d] ;
				}
			}
			else if (curr->type == 1) {
				for (t = 0 ; t < curr->num_pix ; ++t)
					priv->view[detn][t] += curr->int_frames[d*curr->num_pix + t] * prob[curr->num_data_prev+d] ;
			}
			else if (curr->type == 2) { // Gaussian EMC update without scaling
				for (t = 0 ; t < curr->num_pix ; ++t)
				if (det[detn].mask[t] < 2)
					priv->view[detn][t] += curr->frames[d*curr->num_pix + t] * prob[curr->num_data_prev+d] ;
			}
		}
